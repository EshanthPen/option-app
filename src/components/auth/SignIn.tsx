...
export default function SignIn({ onAuthSuccess, onGuestMode }) {
  const [keepSignedIn, setKeepSignedIn] = useState(true);

  useEffect(() => {
    AsyncStorage.setItem('keepSignedIn', keepSignedIn.toString());
  }, [keepSignedIn]);
  ...
  return (
    <div className="flex h-[100dvh] w-full items-center justify-center bg-background text-foreground font-sans">
      <motion.div 
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-muted-foreground mt-2 text-center text-sm font-mono">
        ...
        <div className="mt-6 flex items-center justify-center">
          <label className="flex items-center gap-2 text-sm text-muted-foreground cursor-pointer select-none hover:text-foreground transition-colors">
            <input 
              type="checkbox" 
              className="accent-primary w-4 h-4 rounded border-border bg-transparent focus:ring-primary focus:ring-offset-background"
